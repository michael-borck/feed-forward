...
def test_score_category_disabled_rule_ignored():
    rules = [
        _rule("flesch", {"type": "linear", "in": [0, 100], "out": [0, 100]}),
        _rule(
            "vocab", {"type": "linear", "in": [0, 100], "out": [0, 100]}, enabled=False
        ),
    ]
...
def test_estimate_scores_for_draft():
    ...
    signal_rules.insert(
        SignalRule(
            rubric_category_id=2,
            signal_source="document-analyser",
            signal_name="vocabulary_richness",
            transform=lin,
            weight=1.0,
            enabled=True,
        )
...
def test_suggest_matches_keywords():
    assert [
        r.signal_name for r in signal_scorer.suggest_rules_for_category("Clarity")
    ] == ["flesch_score"]
    assert [
        r.signal_name
        for r in signal_scorer.suggest_rules_for_category("Structure & Organisation")
    ] == ["paragraph_count", "sentence_variety"]
    assert [
        r.signal_name for r in signal_scorer.suggest_rules_for_category("Vocabulary")
    ] == ["vocabulary_richness"]
    assert [
        r.signal_name for r in signal_scorer.suggest_rules_for_category("Writing style")
...
def test_suggest_matches_tone_to_sentiment():
    assert [
        r.signal_name for r in signal_scorer.suggest_rules_for_category("Tone")
    ] == ["sentiment_positive"]
    assert [
        r.signal_name for r in signal_scorer.suggest_rules_for_category("Engagement")
    ] == ["sentiment_positive"]
...
def test_category_estimates_auto_match():
    ...
    cats = [
        SimpleNamespace(id=1, name="Clarity"),
        SimpleNamespace(id=2, name="Argument depth"),
    ]
...
def test_category_estimates_prefers_persisted():
    ...
    assert (
        est[1]["score"] == 45.0
    )  # persisted linear (45) overrides suggested band (72)
