...
def post(session, assignment_id: int, content: str, version: int):
    ...
    # Trigger AI feedback generation pipeline in background
    try:
        from app.utils.feedback_pipeline import process_draft_submission

        process_draft_submission(draft_id, background=True)
        logger.info(f"Started AI feedback processing for draft {draft_id}")
    except Exception as e:
        logger.error(
            f"Failed to start feedback processing for draft {draft_id}: {e!s}"
        )
        # Continue even if feedback processing fails to start
                            ...
                            Tr(
                                ...
                                Td(
                                    Div(
                                        H4(
                                            assignment_data["assignment"].title,
                                            cls="font-semibold text-indigo-800",
                                        ),
                                        P(
