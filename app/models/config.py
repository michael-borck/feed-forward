...
if system_config not in db.t:
    system_config.create(dict(key=str, value=str, description=str), pk="key")
...
if aggregation_methods not in db.t:
    aggregation_methods.create(
        dict(id=int, name=str, description=str, is_active=bool), pk="id"
    )
...
if assignment_model_runs not in db.t:
    assignment_model_runs.create(
        dict(id=int, assignment_setting_id=int, ai_model_id=int, num_runs=int), pk="id"
    )
