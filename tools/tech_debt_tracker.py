...
class TechDebtTracker:
    ...
    def _console_report(self) -> None:
        ...
        summary_table.add_row(
            "Critical Severity", str(self.stats.get("severity_critical", 0))
        )
        summary_table.add_row("High Severity", str(self.stats.get("severity_high", 0)))
        summary_table.add_row(
            "Medium Severity", str(self.stats.get("severity_medium", 0))
        )
        ...
        for debt_type, count in sorted(types.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]:
        ...
        worst_files = sorted(file_debt.items(), key=lambda x: len(x[1]), reverse=True)[
            :10
        ]
    ...
    def _markdown_report(self) -> None:
        ...
        for file_path, items in sorted(file_debt.items()):
            ...
            for item in sorted(items, key=lambda x: x.line_number):
                ...
                print(
                    f"- Line {item.line_number}: {severity_emoji} **{item.debt_type}**"
                )
                if item.message:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Track technical debt in Python codebase"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--exclude", nargs="+", help="Patterns to exclude from scanning"
    )
    parser.add_argument(
        "--severity",
    ...
    if args.severity != "all":
        ...
        tracker.debt_items = [
            item
            for item in tracker.debt_items
