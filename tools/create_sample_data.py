...
from app.utils.auth import get_password_hash, generate_token_expiry
from app.models.user import User, Role, users, db
...
from app.models.feedback import AIModel, Draft, ModelRun, model_runs, ai_models, drafts
from app.models.feedback import CategoryScore, FeedbackItem, AggregatedFeedback
...
def clear_all_tables():
    ...
    for table in tables:
        try:
            ...
            print(f"  Error clearing {table.name}: {str(e)}")
...
def create_users():
    ...
    for email, name, department in instructors:
        instructor = User(
            email=email,
            name=name,
            password=get_password_hash("Instructor123!"),  # In production, use a proper password
    ...
    for i, name in enumerate(student_names):
        ...
        student = User(
            email=email,
            name=name,
            password=get_password_hash("Student123!"),  # In production, use a proper password
            role=Role.STUDENT,
            verified=True,
            verification_token="",
            approved=True,
...
def create_assignments():
    ...
    for i, data in enumerate(assignments_data, 1):
        ...
        print(f"  Created assignment: {assignment.title} for course ID {assignment.course_id}")
...
def create_rubrics():
    ...
    rubric_data = [
        {
            ...
            "categories": [
                {"name": "Code Correctness", "description": "Does the code run without errors and produce the expected output?", "weight": 0.4},
                {"name": "Code Style", "description": "Is the code well-organized, properly indented, and well-commented?", "weight": 0.3},
                {"name": "Algorithm Efficiency", "description": "Is the code efficient and does it use appropriate algorithms?", "weight": 0.3}
            ]
        },
        {
            "assignment_id": 2,  # Functions and Methods
            "categories": [
                {"name": "Function Design", "description": "Are functions properly designed with appropriate parameters and return values?", "weight": 0.4},
                {"name": "Code Quality", "description": "Is the code modular, reusable, and maintainable?", "weight": 0.3},
                {"name": "Documentation", "description": "Are functions and methods properly documented?", "weight": 0.3}
            ]
        },
        {
            "assignment_id": 3,  # Data Visualization Project
            "categories": [
                {"name": "Data Analysis", "description": "Is the data analysis thorough and insightful?", "weight": 0.4},
                {"name": "Visualization Quality", "description": "Are visualizations clear, appropriate, and properly labeled?", "weight": 0.4},
                {"name": "Insights and Conclusions", "description": "Are insights from the data clearly articulated?", "weight": 0.2}
            ]
        },
        {
            "assignment_id": 4,  # Software Design Patterns
            "categories": [
                {"name": "Pattern Selection", "description": "Are appropriate design patterns selected for the problem?", "weight": 0.3},
                {"name": "Implementation", "description": "Are the design patterns correctly implemented?", "weight": 0.4},
                {"name": "Explanation", "description": "Is the reasoning behind pattern selection well-explained?", "weight": 0.3}
            ]
        }
    ]
    ...
    for data in rubric_data:
        ...
        print(f"  Created rubric and {len(data['categories'])} categories for assignment ID {data['assignment_id']}")
        rubric_id += 1
...
def create_system_config():
    ...
    aggregation_methods_data = [
        {"id": 1, "name": "Average", "description": "Simple average of all model scores", "is_active": True},
        {"id": 2, "name": "Weighted Average", "description": "Weighted average based on model confidence", "is_active": True},
        {"id": 3, "name": "Maximum", "description": "Take the highest score from any model", "is_active": False}
    ]
    ...
    feedback_styles_data = [
        {"id": 1, "name": "Detailed", "description": "Provides detailed feedback on all aspects", "is_active": True},
        {"id": 2, "name": "Concise", "description": "Provides brief, to-the-point feedback", "is_active": True},
        {"id": 3, "name": "Constructive", "description": "Focuses on improvement opportunities", "is_active": True}
    ]
    ...
    mark_display_options_data = [
        {"id": 1, "display_type": "numeric", "name": "Percentage", "description": "Display as percentage", "icon_type": "", "is_active": True},
        {"id": 2, "display_type": "icon", "name": "Stars", "description": "Display as star rating", "icon_type": "star", "is_active": True},
        {"id": 3, "display_type": "hidden", "name": "Hidden", "description": "Don't show scores to students", "icon_type": "", "is_active": True}
    ]
    ...
    system_config_data = [
        {"key": "institution_name", "value": "Curtin University", "description": "Name of the institution"},
        {"key": "feedback_delay_min", "value": "30", "description": "Minimum delay in seconds before showing feedback"},
        {"key": "feedback_delay_max", "value": "120", "description": "Maximum delay in seconds before showing feedback"},
        {"key": "default_aggregation_method", "value": "1", "description": "Default aggregation method ID"},
        {"key": "default_feedback_style", "value": "1", "description": "Default feedback style ID"},
        {"key": "default_mark_display", "value": "1", "description": "Default mark display option ID"}
    ]
    ...
    print("  Created system configuration, aggregation methods, feedback styles, and mark display options")
...
"""
    ...
    for assignment_id, content in draft_contents.items():
        ...
        for enrollment in course_enrollments[:4]:  # Limit to first 4 students
            ...
            drafts.insert(draft)
            print(f"  Created draft for assignment ID {assignment_id}, student {student_email}")
            draft_id += 1
            ...
            if random.random() > 0.5:
                ...
                drafts.insert(draft)
                print(f"  Created second draft for assignment ID {assignment_id}, student {student_email}")
                draft_id += 1
...
def create_sample_feedback():
    ...
    for draft in ready_drafts:
        ...
        setting = next((s for s in assignment_settings() if s.assignment_id == assignment_id), None)
        if not setting:
            continue
        ...
        for category in categories:
            ...
            category_scores_list = [score for score in category_scores() 
            ...
            if category_scores_list:
                aggregated_score = sum(score.score for score in category_scores_list) / len(category_scores_list)
            else:
