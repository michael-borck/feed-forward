    ...
    def refactor_imports(self, source: str) -> tuple[str, list[str]]:
        ...
        if import_line_idx is not None:
            ...
            for component in used_components:
                ...
                pattern = (
                    r'\b(?<!fh\.)(?<!["\'])(' + re.escape(component) + r")(?=\s*\()"
                )
                replacement = r"fh.\1"
...
def main():
    ...
    parser.add_argument(
        "path", nargs="?", default=".", help="Path to file or directory to refactor"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Show detailed output including diffs"
    )
    ...
    if (
        ...
        and path.is_dir()
        and Confirm.ask(
            "[yellow]Create backup before refactoring?[/yellow]", default=True
        )
    ):
    ...
    # Create refactorer
    refactorer = FastHTMLImportRefactorer(dry_run=args.dry_run, verbose=args.verbose)
    ...
    if not args.dry_run and refactorer.stats["files_modified"] > 0:
        ...
        console.print(
            "4. Commit changes: git add -A && git commit -m 'Refactor FastHTML imports'"
        )
